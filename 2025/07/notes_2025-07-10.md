...
-- check other PRT `p` values for consistency
SELECT *
  FROM (
          (SELECT
             formatted_id,
...
-- used to calculate geomean from ic50 values to double check
WITH t AS (
    SELECT 
        formatted_id,
        ic50,
...
### ✅ Tested PRT Numbers for Validation
